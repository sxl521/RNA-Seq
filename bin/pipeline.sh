...
$bowtie2 -x $rRNA -1 $PWD/Trimmomatic/$sample/rRNA/$sample"_"1.test.fq.gz -2 $PWD/Trimmomatic/$sample/rRNA/$sample"_"2.test.fq.gz --end-to-end --sensitive -p 8 --phred33 --no-mixed -X 500 -S $PWD/Trimmomatic/$sample/rRNA/$sample.sam 2 >$PWD/Trimmomatic/$sample/rRNA/$sample.rRNA.stat.txt
rm $PWD/Trimmomatic/$sample/rRNA/$sample"_"1.test.fq.gz $PWD/Trimmomatic/$sample/rRNA/$sample"_"2.test.fq.gz $PWD/Trimmomatic/$sample/rRNA/$sample.sam

##判断rRNA含量,如果rRNA含量大于等于1%，则剔除rRNA；
rRNA_rate=`tail -n $PWD/Trimmomatic/$sample/rRNA/$sample.rRNA.stat.txt|awk -F '%' '{print $1}'`
rRNA_rate=`echo $rRNA_rate/1|bc`
if [ $rRNA_rate -ge 1 ]; then
  $bowtie2 -x $rRNA -1 $PWD/Trimmomatic/$sample/$sample"_"1.clean.fq.gz -2 $PWD/Trimmomatic/$sample/$sample"_"2.clean.fq.gz --end-to-end --sensitive -p 8 --phred33 --un-conc-gz $PWD/Trimmomatic/$sample/rRNA/$sample.unmapped.gz -X 500 -S $PWD/Trimmomatic/$sample/rRNA/$sample.sam 2 >$PWD/Trimmomatic/$sample/rRNA/$sample.rRNA.stat.txt
